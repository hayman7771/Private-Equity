# Investment Committee Memo: Midwest Industrial Solutions (“MIS”)

## 1. Executive Summary
Recommendation: **Proceed to LOI**  
Rationale: Sticky revenue base, strong FCF, credible operational uplift, and platform potential.

## 2. Transaction Overview
- Founder seeking partial liquidity and transition
- Potential for majority recapitalization
- Clean balance sheet (leverageable to 3.5–4.0x EBITDA)

## 3. Business Overview
- Niche engineered components
- 40+ year operating history
- Deep customer relationships
- Mission-critical products with qualification barriers

## 4. Industry Overview
- Fragmented supplier base
- OEM reshoring ? tailwinds
- Moderate but steady sector growth

## 5. Financial Summary
- 3-year revenue CAGR: ~6–7%
- EBITDA margin: 14%
- FCF conversion: 80–85%

## 6. Valuation Summary
- Entry: 8–9x EBITDA reasonable  
- DCF EV: \–95M  
- Trading comps: 8–10x  
- Underwriting target: 2.5–3.0x MOIC

## 7. Value Creation Plan
- Lean manufacturing implementation  
- Procurement centralization  
- Pricing optimization  
- Add-on M&A rollups

## 8. Key Risks & Mitigants
- Customer concentration (diligence contract stickiness)
- Cyclicality (hedge with contract mix) 
- Execution (upgrade management bench)

## 9. Recommendation
Proceed to LOI and enter confirmatory diligence.
